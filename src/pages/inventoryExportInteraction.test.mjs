...
assert.match(inventorySource, /buildInventoryCsv\(filteredRows\)/, 'export should include every filtered row across all pages');
...
assert.match(inventorySource, /已导出 \$\{filteredRows\.length\} 条库存风险数据/, 'successful exports should report the exported row count');
