function template(id, label, description, resolvedSource) {
  return { id, label, result: label, description, resolvedSource };
}
...
const templateFactories = {
  warehouse: (source) => [
    ...
    template('warehouse-blocked', '无法切换已升级', `${source} 因目标仓库存或履约条件不足无法切换，已升级主管处理。`, false),
  ],
  ...
  address: (source) => [
    ...
    template('address-closed', '地址无效待升级', `${source} 的地址经核验仍无效，已记录原因并等待升级处理。`, false),
  ],
  platform: (source) => [
    ...
    template('platform-escalated', '已升级技术处理', `${source} 多次同步失败，已升级技术人员继续处理。`, false),
  ],
  refund: (source) => [
  ...
  fallback: (source) => [
    ...
    template('fallback-escalated', '无法处理已升级', `${source} 当前无法继续处理，已升级相关负责人。`, false),
  ],
};
